...
def get_argparser():
    ...
    parser.add_argument(
        "--weak_pth", type=str, default="higher_hrnet48", help="model name"
    )
...
if __name__ == "__main__":
    ...
    if args.local_rank == 0:
        ...
        print(
            "strong_labels : ",
            len(strong_labels["images"]),
            len(strong_labels["annotations"]),
        )

        with open(args.weak_json) as f:
            weak_json = json.load(f)
            print(
                "weak_json : ", len(weak_json["images"]), len(weak_json["annotations"])
            )
        ...
        print("\n=============================================")
        print(
            f" strong_labels       : img {len(strong_labels['images'])} , anno {len(strong_labels['annotations'])}"
        )
        print(
            f" weak_labels         : img {len(weak_json['images'])} , anno {len(weak_json['annotations'])}"
        )
        print(
            f" refined_weak_labels : img {len(weak_json['images'])} , anno {len(refined_weak_results['annotations']) - len(strong_labels['annotations'])}"
        )
        print(
            f" merge_labels        : img {len(refined_weak_results['images'])} , anno {len(refined_weak_results['annotations'])}"
        )
        print("=============================================\n")
