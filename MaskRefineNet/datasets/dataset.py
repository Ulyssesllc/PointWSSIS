...
class TrainSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if type(gt["segmentation"]) == list:
            gt_mask = mask_util.decode(
                mask_util.merge(mask_util.frPyObjects(gt["segmentation"], h, w))
            )
        ...
        if self.transform is not None:
            [cropped_img], [pred_cropped_mask, gt_cropped_mask] = self.transform(
                [cropped_img], [pred_cropped_mask, gt_cropped_mask]
            )
        ...
        if t_cls == 0:
            center_map = torch.zeros(
                (self.num_classes + 1, th, tw), dtype=torch.float32
            )
            ...
            center_map = center_map_gen(
                center_map, int(cx), int(cy), t_cls, self.sigma, self.g
            )
        ...
        cropped_img = torch.cat(
            [cropped_img, center_map, pred_cropped_mask.unsqueeze(0)]
        )
                        ...
                        m = mask_util.decode(
                            mask_util.merge(
                                mask_util.frPyObjects(
                                    l["segmentation"], info["height"], info["width"]
                                )
                            )
                        )
                ...
                for j, l in enumerate(pred_labels):
                    ...
                    if cate in gt_point_per_cls:
                        for gt in gt_point_per_cls[cate]:
                            dist = math.sqrt(
                                (cy - gt["point"][0]) ** 2 + (cx - gt["point"][1]) ** 2
                            )
...
class ValidSet(data.Dataset):
    def __init__(
        ...
        if filtering:
            self.weak_labels = [
                w for w in self.weak_labels if w["image_id"] in gt_image_ids
            ]
    ...
    def __getitem__(self, index):
        ...
        for inst in pred["instances"]:
            ...
            if (
                (crop_ymax - crop_ymin + 1) > self.size_cut
                and (crop_xmax - crop_xmin + 1) > self.size_cut
                and mask.sum() > 0
            ):
                cropped_x = x[
                    :, crop_ymin : crop_ymax + 1, crop_xmin : crop_xmax + 1
                ].clone()
                cropped_mask = torch.from_numpy(
                    mask[crop_ymin : crop_ymax + 1, crop_xmin : crop_xmax + 1]
                ) * (category_id + 1)
                ...
                cropped_x, cropped_mask = self.tensor_resize(
                    cropped_x, cropped_mask, self.val_size
                )
                ...
                center_map = np.zeros(
                    (self.num_classes + 1, crop_h, crop_w), dtype=np.float32
                )
                center_map = center_map_gen(
                    center_map, gt_cx, gt_cy, cropped_cls, self.sigma, self.g
                )
                center_map = torch.from_numpy(center_map)
    ...
    def tensor_resize(self, img, mask, size):
        ...
        img = F.interpolate(
            img[None, ...], (h, w), mode="bilinear", align_corners=False
        ).squeeze(0)
        mask = F.interpolate(
            mask[None, None, ...].float(), (h, w), mode="nearest"
        ).squeeze(0)
