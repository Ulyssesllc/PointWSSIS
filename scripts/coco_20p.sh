...
#     --workers 8 \
#     --save_path ${root}/coco/annotations/instances_train2017_20p_sw.json \
#     --strong_json ${root}/coco/annotations/instances_train2017_20p_s.json \
...
# trainsets="('coco_2017_train_20p_sw',)"
# testsets="('coco_2017_val',)"
# exp_name_sw="SOLOv2_R101_coco20p_sw"
...
OMP_NUM_THREADS=1 python3 -W ignore "${PROJECT_ROOT}/AdelaiDet/tools/train_net.py" \
    ...
    --num-gpus ${ngpus} \
    --eval-only \
    MODEL.WEIGHTS training_dir/${exp_name}/model_0069999.pth \
...
MRN_exp_name="MRN_20p"
gt_json="instances_train2017_20p_s.json"
...
torchrun --standalone --nnodes=1 --nproc_per_node=${ngpus} "${PROJECT_ROOT}/MaskRefineNet/merge_strong_and_refined_weak_labels.py" \
    ...
    --size 256 \
    --weak_pth ${eval_pth} \
    --weak_json ${root}/coco/annotations/instances_train2017_20p_w.json \
    --strong_json ${root}/coco/annotations/instances_train2017_20p_s.json \
    --save_path ${root}/coco/annotations/instances_train2017_20p_sw_refined.json


# step 6
config_file="configs/SOLOv2/R101_3x.yaml"
trainsets="('coco_2017_train_20p_sw_refined',)"
testsets="('coco_2017_val',)"
