...
#     --workers 8 \
#     --save_path ${root}/coco/annotations/instances_train2017_50p_sw.json \
#     --strong_json ${root}/coco/annotations/instances_train2017_50p_s.json \
...
# trainsets="('coco_2017_train_50p_sw',)"
# testsets="('coco_2017_val',)"
# exp_name_sw="SOLOv2_R101_coco50p_sw"
...
OMP_NUM_THREADS=1 python3 -W ignore "${PROJECT_ROOT}/AdelaiDet/tools/train_net.py" \
    ...
    --num-gpus ${ngpus} \
    --eval-only \
    MODEL.WEIGHTS training_dir/${exp_name}/model_0179999.pth \
...
OMP_NUM_THREADS=1 python3 -W ignore "${PROJECT_ROOT}/AdelaiDet/tools/train_net.py" \
    ...
    --num-gpus ${ngpus} \
    --eval-only \
    MODEL.WEIGHTS training_dir/${exp_name}/model_0239999.pth \
...
torchrun --standalone --nnodes=1 --nproc_per_node=${ngpus} "${PROJECT_ROOT}/MaskRefineNet/merge_strong_and_refined_weak_labels.py" \
    ...
    --size 256 \
    --weak_pth ${eval_pth} \
    --weak_json ${root}/coco/annotations/instances_train2017_50p_w.json \
    --strong_json ${root}/coco/annotations/instances_train2017_50p_s.json \
    --save_path ${root}/coco/annotations/instances_train2017_50p_sw_refined.json
